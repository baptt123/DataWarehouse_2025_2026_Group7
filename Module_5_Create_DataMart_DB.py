...
def main():
    ...
    try:
        ...
        send_alert("FL_5: Lỗi kết nối CONTROL DATABASE", str(e), cfg)
        print('2')
        write_log(cfg, None, start_time, 0, 0, "FL_5")
        return
    ...
    if not config_row:
        send_alert("FL_5: Không có config trong CONTROL DB", "Bảng config trống!", cfg)
        print('no config')
        write_log(cfg, None, start_time, 0, 0, "FL_5")
        return
    ...
    # 3. Load dữ liệu kết nối từ control-database 
    # dm_host, dm_username, dm_password, dm_db
    ...
    if missing:
        send_alert("FL_5: Thiếu cấu hình DATAMART", "\n".join(missing), cfg)
        print('3')
        write_log(cfg, id_config, start_time, 0, 0, f"FL_5")
        return
    ...
    if not csv_path:
        send_alert("FL_5, FE_DM: Không tìm thấy file datamart", "Không có file data_mart_*.csv trong D:/Warehouse", cfg)
        print('4')
        write_log(cfg, id_config, start_time, 0, 0, "FL_5 - FE_DM")
        return
    ...
    try:
        ...
        send_alert("FL_5: Lỗi kết nối DATAMART DATABASE", str(e), cfg)
        print('5')
        write_log(cfg, id_config, start_time, 0, 0, "FL_5")
        return
    ...
    # 6. Load file csv D:/Warehouse/
    # data_mart_yyyyMMdd_HHmmss.csv vào table fact_weather_report trong datamart-database
    try:
        ...
        send_alert("FL_5, FE_DM: Lỗi đọc/ghi CSV trong MODULE 5", str(e), cfg)
        print('6')
        write_log(cfg, id_config, start_time, 0, 0, "FL_5 - FE_DM")
        return
    ...
    write_log(cfg, id_config, start_time, total, fail, "SC_5")
