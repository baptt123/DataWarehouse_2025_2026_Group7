...
def main():
    ...
    try:
        ...
        write_log(cfg, None, start_time, 0, 0, f"MODULE 1 FAIL: CONTROL CONNECTION")
        return
    ...
    if not config_row:
        send_alert("Không tìm thấy cấu hình trong CONFIG", "Bảng config trống!", cfg)
        write_log(cfg, None, start_time, 0, 0, "MODULE 1 FAIL: NO CONFIG RECORD")
        return
    ...
    if  stg_host is None or stg_username is None or stg_password is None or stg_db is None:
        ...
        send_alert("Thiếu cấu hình STAGING DATABASE", "\n".join(missing), cfg)
        write_log(cfg, id_config, start_time, 0, 0, f"MODULE 1 FAIL: MISSING STAGING CONFIG ({', '.join(missing)})")
        return
    ...
    try:
        ...
        send_alert("Lỗi kết nối STAGING DATABASE", str(e), cfg)
        write_log(cfg, id_config, start_time, 0, 0, f"MODULE 1 FAIL: STAGING CONNECTION")
        return
    ...
    if not locations:
        send_alert("Không có location trong stg_location", "Module dừng.", cfg)
        write_log(cfg, id_config, start_time, 0, 0, "MODULE 1 STOP: NO LOCATION")
        return
    ...
    if not api_key or not base_url:
        send_alert("Thiếu API KEY hoặc BASE_URL", f"api_key={api_key}, base_url={base_url}", cfg)
        write_log(cfg, id_config, start_time, 0, 0, "MODULE 1 FAIL: MISSING API CONFIG")
        return
    ...
    try:
        ...
        send_alert("Lỗi ghi file CSV tại MODULE 1", str(e), cfg)
        write_log(cfg, id_config, start_time, 0, 0, f"MODULE 1 FAIL: CSV WRITE ERROR")
        return
    ...
    # 8) Tiến hành ghi log
    write_log(cfg, id_config, start_time, total, fail, "MODULE 1 SUCCESS")
